<template>
  <NuxtLink 
  class="block transition-shadow duration-300 ease-in-out shadow-lg hover:shadow-2xl">
    <div   class="w-full h-64 overflow-hidden border-b-4 border-red-700 duration-300 ease-in-out transform hover:scale-105 group-hover:bg-red-600 transition duration-300 ease-in-out cursor-pointer">
      <NuxtImg fit="cover" @click="openModal(product.image)" :src="product.image" :placeholder="[50, 75]" class="w-full object-cover" />
    </div>
    <div class="py-4 bg-transparent p-2">
      <h3 class="lg:text-[18px] sm:text-[12px] md:text-[14px] mb-3 line-clamp-1">
        {{ product.title }}
      </h3>
      <p class="font-light text-slate-300 sm:text-[10px] md:text-[12px] lg:text-[16px] hyphens-auto text-ellipsis overflow-hidden leading-relaxed line-clamp-3 mb-5">
        {{ product.description }}
      </p>
